...
int main() {
    auto [boards, draw_pool, file, first_line, winners] = std::tuple{std::vector<std::array<std::array<std::tuple<int, bool>, 5>, 5>>(), std::vector<int>(), std::ifstream("input.txt"), std::string(""), std::unordered_set<int>()};
    ...
    for (auto [line, board, x, y] = std::tuple{std::string(), std::array<std::array<std::tuple<int, bool>, 5>, 5>(), 0, 0}; !file.eof() && file >> line;) {
        ...
        if (x == 5) {x = 0; y++;};
        if (y == 5) {boards.push_back(board); x = 0, y = 0;};
    }

    for (auto &num : draw_pool)
        for (int unsigned i = 0; i < boards.size(); i++)
            if (!winners.contains(i) && mark_in_board(boards, num, i, winners)) std::cout << "winner in " << i << " : " << unmarked_sum(boards.at(i)) * num << std::endl;
}